...
export const initCollection = async (connectionWrapper: WrappedConnection, payer: Keypair) => {
    ...
    let tx = new Transaction().add(collectionMeatadataIX).add(collectionMasterEditionIX).add(sizeCollectionIX);
    try {
        ...
        console.log(
            'Successfull created NFT collection with collection address: ' + collectionMint.publicKey.toBase58(),
        );
        return {
            collectionMint,
...
export const getCompressedNftId = async (tree: PublicKey, leafIndex: number) => {
    const node = new BN.BN(leafIndex);
    const [assetId] = await PublicKey.findProgramAddress(
        [Buffer.from('asset', 'utf8'), tree.toBuffer(), Uint8Array.from(node.toArray('le', 8))],
        BUBBLEGUM_PROGRAM_ID,
    );
    return assetId;
};
